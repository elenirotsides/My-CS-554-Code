<template>
    <b-container fluid>
        ...
        <nav v-if="this.loading === false && this.badRequest === false">
            <router-link
                v-if="this.page > 0"
                :to="{ name: 'charactersList', params: { page: parseFloat(this.$route.params.page) - 1 } }"
                class="pagelink"
            >
                Previous Page
            </router-link>
        ...
        <b-row v-else>
            <b-col cols="6" md="2" v-for="(character, index) in characters" :key="index">
                <div>
                    <h1></h1>
                    <h2></h2>
                    <h3></h3>
                    <router-link :to="{ name: 'characters', params: { id: character.id } }">
                        <b-card-img
                            ...
                            :alt="`${character.name}`"
                        ></b-card-img>
                        <b-card :title="`${character.name}`" class="mb-2 name"></b-card>
                    </router-link>
                </div>
            </b-col>
        </b-row>
...
export default {
    ...
    created() {
        this.getCharList(this.$route.params.page);
    },
    watch: {
        $route() {
            this.getCharList(this.$route.params.page);
